...
function getCraftableRecipes(recipes, inventory) {
    // bot.recipesFor() has a mineflayer bug where it can incorrectly filter out
    // valid recipes (e.g. for non-oak variants). bot.recipesAll() returns every
    // matching recipe unfiltered by the bot's inventory, so we do that filtering
    // ourselves here based on what's actually in the inventory.
    if (!recipes) return [];
    return recipes.filter(recipe => {
        const requiredIngredients = mc.ingredientsFromPrismarineRecipe(recipe);
        const { num } = mc.calculateLimitingResource(inventory, requiredIngredients);
        return num >= 1;
    });
}
...
export async function craftRecipe(bot, itemName, num=1) {
    ...
    let recipes = getCraftableRecipes(bot.recipesAll(mc.getItemId(itemName), null, null), world.getInventoryCounts(bot));
    let craftingTable = null;
    const craftingTableRange = 16;
    placeTable: if (!recipes || recipes.length === 0) {
        recipes = getCraftableRecipes(bot.recipesAll(mc.getItemId(itemName), null, true), world.getInventoryCounts(bot));
        ...
        if (craftingTable === null){
            ...
            if (hasTable) {
                ...
                craftingTable = world.getNearestBlock(bot, 'crafting_table', craftingTableRange);
                if (craftingTable) {
                    recipes = getCraftableRecipes(bot.recipesAll(mc.getItemId(itemName), null, craftingTable), world.getInventoryCounts(bot));
                    placedTable = true;
                }
            }
            else {
    ...
    if (!recipes || recipes.length === 0) {
        const bestVariant = mc.getBestCraftingRecipe(itemName, world.getInventoryCounts(bot));
        log(bot, `You do not have the resources to craft a ${itemName}. It requires: ${Object.entries(bestVariant[0]).map(([key, value]) => `${key}: ${value}`).join(', ')}.`);
        if (placedTable) {
