...
export async function craftRecipe(bot, itemName, num=1) {
    ...
    // Resolve which ingredient variant (e.g. which wood type) we intend to craft
    // with, once, using our inventory-aware selection. This becomes the single
    // source of truth for BOTH picking the actual recipe to hand to mineflayer
    // AND any failure messaging - so the two can never disagree with each other
    // again (previously, mineflayer's own recipe matching and our own
    // wood-aware messaging were two independent systems that could pick
    // different variants).
    const currentInventory = world.getInventoryCounts(bot);
    const chosenVariant = mc.getBestCraftingRecipe(itemName, currentInventory);

    // Does a raw mineflayer/prismarine recipe match the ingredient variant we
    // resolved above? Compares by ingredient name and count (using absolute
    // value, since prismarine-recipe stores shapeless ingredient counts as
    // negative numbers - see ingredientsFromPrismarineRecipe).
    const matchesChosenVariant = (recipe) => {
        if (!chosenVariant) return true; // no inventory preference; accept anything mineflayer offers
        const wanted = chosenVariant[0];
        const got = mc.ingredientsFromPrismarineRecipe(recipe);
        const gotKeys = Object.keys(got).filter(k => got[k] !== 0);
        const wantedKeys = Object.keys(wanted);
        if (gotKeys.length !== wantedKeys.length) return false;
        return wantedKeys.every(key => key in got && Math.abs(got[key]) === wanted[key]);
    };

    // From a list of recipes mineflayer says we can afford, prefer whichever
    // one matches our resolved variant (e.g. the dark_oak_planks recipe when
    // that's what's in inventory). If none match exactly - which would mean
    // our model and mineflayer's underlying recipe data disagree - fall back
    // to mineflayer's own first result so crafting still works, but log it
    // so the mismatch is visible rather than silently masked.
    const pickRecipe = (recipeList) => {
        if (!recipeList || recipeList.length === 0) return null;
        const exact = recipeList.find(matchesChosenVariant);
        if (exact) return exact;
        if (chosenVariant) {
            log(bot, `Note: crafting ${itemName} with an available recipe that didn't exactly match the resolved ingredient variant.`);
        }
        return recipeList[0];
    };
    ...
    let recipe = pickRecipe(recipes);
    let craftingTable = null;
    const craftingTableRange = 16;
    placeTable: if (!recipe) {
        recipes = bot.recipesFor(mc.getItemId(itemName), null, 1, true);
        recipe = pickRecipe(recipes);
        if (!recipe) break placeTable; //Don't bother going to the table if we don't have the required resources.
        ...
        if (craftingTable === null){
            ...
            if (hasTable) {
                ...
                if (craftingTable) {
                    ...
                    recipe = pickRecipe(recipes) || recipe;
                    placedTable = true;
                }
            }
            else {
    ...
    if (!recipe) {
        const failureVariant = chosenVariant || mc.getItemCraftingRecipes(itemName)[0];
        log(bot, `You do not have the resources to craft a ${itemName}. It requires: ${Object.entries(failureVariant[0]).map(([key, value]) => `${key}: ${value}`).join(', ')}.`);
        if (placedTable) {
...
export async function goToGoal(bot, goal) {
    ...
    // explicitly allow scaffolding/tower-climbing so the pathfinder itself
    // plans over 1-block gaps/rises instead of relying on reactive unstuck logic
...
function startDoorInterval(bot) {
    /**
     * Start a helper interval that proactively resolves common local movement
     * stalls (ledges, doors, dead-ends) while the pathfinder is en route,
     * instead of waiting for the much slower mode-level 'unstuck' fallback.
     * Escalates through cheap fixes first:
     *   ~0.6s stalled -> jump pulse (handles ledges/slabs pathfinder fumbles physically)
     *   ~1.2s stalled -> try opening a nearby door/gate/trapdoor (original behavior)
     *   ~2.0s stalled -> place a scaffold block underfoot/ahead if carrying blocks (capped)
     *   ~3.0s stalled -> sidestep to break out of a wedge, then let mode-level unstuck take over
     * @param {MinecraftBot} bot, reference to the minecraft bot.
    ...
    let stuck_time = 0;
    let did_jump_pulse = false;
    let scaffold_attempts = 0;
    const max_scaffold_attempts = 3; // avoid tower-spamming blocks
    let did_sidestep = false;

    const tryJumpAssist = () => {
        // one-tick jump pulse; helps the bot pop over ledges/1-block lips
        // that the physics step sometimes fumbles even on a valid path.
        bot.setControlState('jump', true);
        setTimeout(() => bot.setControlState('jump', false), 250);
    };

    const tryScaffold = async () => {
        if (scaffold_attempts >= max_scaffold_attempts) return false;
        const blockName = bot.inventory.items().find(item => mc.getBlockId(item.name) != null)?.name;
        if (!blockName) return false; // nothing placeable to scaffold with
        const feetPos = bot.entity.position.floored();
        const ahead = bot.blockAt(feetPos.offset(0, 0, 0));
        const below = bot.blockAt(feetPos.offset(0, -1, 0));
        // only scaffold if we're not already standing on solid ground (i.e. we're
        // stuck against a rise/gap), so we don't randomly tower up on flat ground
        if (below && below.boundingBox === 'block') return false;
        try {
            scaffold_attempts++;
            await placeBlock(bot, blockName, feetPos.x, feetPos.y - 1, feetPos.z, 'top', true);
            return true;
        } catch (err) {
            return false;
        }
    ...
    const trySidestep = () => {
        // nudge left/right briefly to break out of a wedge between two blocks
    ...
    const doorCheckInterval = setInterval(() => {
        ...
        if (stuck_time > 600 && !did_jump_pulse) {
            tryJumpAssist();
            did_jump_pulse = true;
        }
        ...
        if (stuck_time > 2000) {
            tryScaffold(); // fire-and-forget; capped internally
        }

        if (stuck_time > 3000 && !did_sidestep) {
            trySidestep();
            did_sidestep = true;
            // beyond this point we let the mode-level 'unstuck' fallback
            // (modes.js) take over if the sidestep doesn't resolve it
        }
...
export async function avoidEnemies(bot, distance=16, max_duration_ms=15000) {
    /**
     * Move a given distance away from all nearby enemy mobs.
     * Bounded by max_duration_ms so a single call can never loop indefinitely
     * (e.g. when enemies keep respawning/wandering back into range at night) -
     * without this cap, callers like the 'cowardice' mode could get stuck
     * fleeing forever and never resume whatever they were doing before.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @param {number} distance, the distance to move away.
     * @param {number} max_duration_ms, hard cap on how long this call will keep fleeing.
     * @returns {Promise<boolean>} true if the bot moved away, false otherwise.
     * @example
    ...
    while (enemy) {
        if (Date.now() - start_time > max_duration_ms) {
            log(bot, `Gave up fleeing after ${Math.round(max_duration_ms / 1000)}s; enemy still nearby but avoiding an endless panic loop.`);
            break;
        }
