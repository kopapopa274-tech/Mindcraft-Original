...
export const actionsList = [
    ...
    },
    {
        ...
        perform: runAsAction(async (agent, name) => {
            ...
            // items despawn 5 minutes after dropping; if this is a death
            // location and that window has passed, don't grind toward an
            // empty spot (which is what used to feed the cowardice/panic loop)
            const ITEM_DESPAWN_MS = 5 * 60 * 1000;
            const age_ms = agent.memory_bank.getPlaceAgeMs(name);
            if (name === 'last_death_position' && age_ms != null && age_ms > ITEM_DESPAWN_MS) {
                skills.log(agent.bot, `It's been over 5 minutes since death; dropped items have likely despawned, so I won't go back to "${name}".`);
                return;
            }
