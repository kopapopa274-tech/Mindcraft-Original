...
const modes_list = [
    ...
    },
    {
        ...
        stuck_time: 0,
        last_time: Date.now(),
        // proactive assist in goToGoal (jump-pulse/door/scaffold/sidestep) now
        // resolves most local stalls within ~3s, so this fallback net can
        // trigger sooner than the old 20s without being trigger-happy
        max_stuck_time: 14,
        prev_dig_block: null,
        update: async function (agent) {
            if (agent.isIdle()) { 
    ...
    },
    {
        ...
        on: true,
        active: false,
        // bounded retrigger budget: prevents an infinite flee/retry loop when
        // repeatedly returning to a mob-infested area (e.g. retrieving items
        // after death at night). After too many flees against the same
        // interrupted action within a short window, cowardice backs off for
        // a cooldown period and lets self_defense or the action itself proceed.
        flee_count: 0,
        flee_window_start: 0,
        flee_window_ms: 30000,
        max_flees_per_window: 3,
        cooldown_until: 0,
        update: async function (agent) {
            const now = Date.now();
            if (now < this.cooldown_until) return; // back off, let other modes/actions run
            const enemy = world.getNearestEntityWhere(agent.bot, entity => mc.isHostile(entity), 16);
            if (enemy && await world.isClearPath(agent.bot, enemy)) {
                if (now - this.flee_window_start > this.flee_window_ms) {
                    // window expired, start counting fresh
                    this.flee_window_start = now;
                    this.flee_count = 0;
                }
                this.flee_count++;
                if (this.flee_count > this.max_flees_per_window) {
                    // fled too many times in a row against the same recurring
                    // threat; cool down instead of fleeing forever
                    say(agent, `Too many enemies to keep running, holding my ground for a bit.`);
                    this.cooldown_until = now + this.flee_window_ms;
                    this.flee_count = 0;
                    return;
                }
