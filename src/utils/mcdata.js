...
/**
 * Given a list of recipe variants (as returned by getItemCraftingRecipes) and
 * the resources currently available, pick the variant that best matches what's
 * already on hand (e.g. prefer a birch_planks recipe over the default
 * oak_planks one if the crafter already has birch_planks), instead of blindly
 * always using variant [0] (which is sorted to prefer oak/common items - a
 * reasonable default only for callers with no inventory context).
 * @param {Array} recipes - array of [ingredientsObj, resultObj] as returned by getItemCraftingRecipes
 * @param {Object.<string,number>} availableItems - resources currently available (inventory, leftovers, etc, combined)
 * @returns {[Object, Object]|null} the best matching recipe variant, or null if recipes is empty/null
 */
export function selectBestRecipeVariant(recipes, availableItems = {}) {
    if (!recipes || recipes.length === 0) return null;
    if (recipes.length === 1) return recipes[0];

    let best = recipes[0];
    let bestAffordableCount = -Infinity;
    for (const variant of recipes) {
        const [ingredients] = variant;
        let affordableCount = Infinity;
        for (const ingredientName in ingredients) {
            const have = availableItems[ingredientName] || 0;
            const need = ingredients[ingredientName];
            affordableCount = Math.min(affordableCount, need > 0 ? have / need : Infinity);
        }
        if (affordableCount > bestAffordableCount) {
            bestAffordableCount = affordableCount;
            best = variant;
        }
    }
    // nothing is fully or partially affordable with current resources -
    // fall back to the default (oak/common-item-biased) ordering, since
    // there's no inventory signal to prefer any particular variant
    if (bestAffordableCount <= 0) return recipes[0];
    return best;
}

/**
 * Convenience wrapper: get every recipe variant for itemName and return
 * whichever one best matches availableItems.
 * @param {string} itemName
 * @param {Object.<string,number>} availableItems
 * @returns {[Object, Object]|null}
 */
export function getBestCraftingRecipe(itemName, availableItems = {}) {
    return selectBestRecipeVariant(getItemCraftingRecipes(itemName), availableItems);
}
...
function craftItem(item, count, inventory, leftovers, crafted = { required: {}, steps: [], leftovers: {} }) {
    ...
    for (const k in inventory) availableForSelection[k] = (availableForSelection[k] || 0) + inventory[k];
    for (const k in leftovers) availableForSelection[k] = (availableForSelection[k] || 0) + leftovers[k];
    const recipe = getBestCraftingRecipe(item, availableForSelection);
    if (!recipe) {
